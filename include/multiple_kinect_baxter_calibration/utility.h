...
namespace utility
{
  ...
  // due to PCL issues we are not able to use C++ 11 std. Let's define our own
  // functions
  ...
  {
    std::vector<int> positions;  // holds all the positions that sub occurs
                                 // within str
  ...
  inline void
  removeNanAndConvertPointCloudToRGB(pcl::PointCloud<pcl::PointXYZRGBA>& in,
                                     pcl::PointCloud<pcl::PointXYZRGB>& out,
  ...
  cv::Mat getImageFromPCLViewer(pcl::visualization::PCLVisualizer* viewer)
  {
    ...
    unsigned char* pixels =
        render->GetRGBACharPixelData(0, 0, cols - 1, rows - 1, 1);
  ...
  void getPointCloudFromMsg(const sensor_msgs::PointCloud2& msg,
                            ...
                            float min_z = 0.5, float max_z = 5.0)
  {
