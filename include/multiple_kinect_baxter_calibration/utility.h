...
template <typename T>
std::vector<std::vector<T> > hstack(
...
inline void PointCloudXYZRGBtoXYZHSV(
    pcl::PointCloud<pcl::PointXYZRGB>& in, pcl::PointCloud<pcl::PointXYZHSV>& out)
{
    out.width = in.width;
...
inline void PointCloudXYZHSVtoXYZRGB(
    pcl::PointCloud<pcl::PointXYZHSV>& in, pcl::PointCloud<pcl::PointXYZRGB>& out)
{
    out.width = in.width;
    out.height = in.height;
    out.points.resize(in.points.size());
...
inline void removeNanAndConvertPointCloudToRGB(pcl::PointCloud<pcl::PointXYZRGBA>& in,
    pcl::PointCloud<pcl::PointXYZRGB>& out, float min_z, float max_z)
{
    ...
    for (size_t i = 0; i < in.points.size(); i++)
    {
        if (!pcl_isfinite(in.points[i].x) || !pcl_isfinite(in.points[i].y)
...
cv::Mat getImageFromPCLViewer(pcl::visualization::PCLVisualizer* viewer)
{
    ...
    unsigned char* pixels = render->GetRGBACharPixelData(0, 0, cols - 1, rows - 1, 1);
...
bool writeCSV(std::string file_name, std::string header, std::vector<std::vector<float> > data,
    std::string& error)
{
    try
    {
...
bool getCameraParametersPCL(
    const std::vector<std::string>& camera, pcl::visualization::Camera& new_camera)
{
    // look for '/' as a separator
    if (camera.size() != 7)
    {
        pcl::console::print_error("[PCLVisualizer::getCameraParameters] Camera parameters "
