/**
 * data_collector_multi.cpp: class file for collecting data from multiple kinects
 * Author: Ravi Joshi
 * Date: 2018/02/28
 */
...
private:
	...
	void callback(const baxter_core_msgs::EndpointStateConstPtr& ee_msg,
			...
			const sensor_msgs::PointCloud2ConstPtr& pc3_msg);
public:
	DataCollectorMulti();
};

void DataCollectorMulti::recordBallPositionWrtBaxter(
		baxter_core_msgs::EndpointStateConstPtr ee_msg) {
...
void DataCollectorMulti::recordBallPositionWrtKinect(
		pcl::ModelCoefficients spheres_coeff[]) {
...
void DataCollectorMulti::baxterArmMotionStatusCallback(
...
void DataCollectorMulti::saveData() {
	...
	if (!status)
		ROS_ERROR_STREAM(
				"Unable to save tracking data of baxter to CSV file. " << err);

	for (size_t kinect_id = 0; kinect_id < KINECT_COUNT; kinect_id++) {
		std::string kinect_file = data_dir + "/position_wrt_multiple_kinect_"
				+ utility::to_string(kinect_id + 1) + ".csv";
		status = utility::writeCSV(kinect_file, header,
				position_wrt_kinects.at(kinect_id), err);
		if (!status)
			ROS_ERROR_STREAM(
					"Unable to save tracking data of kinect " << kinect_id
							<< " to CSV file. " << err);
	}
}

void DataCollectorMulti::callback(
	...
	for (size_t i = 0; i < 3; i++) {
		std::string file_name =
				"/home/tom/Documents/ravi/Recycle_Bin/del/frame_"
						+ utility::to_string(frame) + "_kinect_"
	...
	for (size_t i = 0; i < KINECT_COUNT; i++) {
		...
		all_success = all_success
				&& sphere_detector->segmentSphere(pc_viewers.at(i), clouds[i],
						segmented_cloud, spheres_coff[i]);

		int j = KINECT_COUNT + i;
		std::string seg_cloud_id = "segmented_cloud" + utility::to_string(i);
		if (!pc_viewers.at(j)->updatePointCloud(segmented_cloud, seg_cloud_id))
			pc_viewers.at(j)->addPointCloud(segmented_cloud, seg_cloud_id);
		pc_viewers.at(j)->spinOnce();
	}
...
void DataCollectorMulti::setWindowPosition(int index) {
...
DataCollectorMulti::DataCollectorMulti() {
	...
	message_filters::Subscriber < sensor_msgs::PointCloud2
			> point_cloud1_sub(nh, pc_topics.at(0), 1);
	message_filters::Subscriber < sensor_msgs::PointCloud2
			> point_cloud2_sub(nh, pc_topics.at(1), 1);
	message_filters::Subscriber < sensor_msgs::PointCloud2
			> point_cloud3_sub(nh, pc_topics.at(2), 1);
	...
	sync.registerCallback(
			boost::bind(&DataCollectorMulti::callback, this, _1, _2, _3, _4));
	...
	baxter_arm_motion_status_sub = nh.subscribe(baxter_arm_motion_status_topic,
			10, &DataCollectorMulti::baxterArmMotionStatusCallback, this);
	ros::spin();
}

int main(int argc, char** argv) {
	ros::init(argc, argv, "data_collector_multi_node",
