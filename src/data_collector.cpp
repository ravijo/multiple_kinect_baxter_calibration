...
void DataCollector::callback(const baxter_core_msgs::EndpointStateConstPtr& ee_msg,
    const sensor_msgs::PointCloud2ConstPtr& pc_msg)
{
    ...
    bool success
        = sphere_detector->segmentSphere(pc_viewers.at(0), cloud, segmented_cloud, sphere_coff);
...
void DataCollector::init(ros::NodeHandle nh)
{
    ...
    pcl_project::RansacParams ransac_params(
        k_neighbors, max_itr, weight, d_thresh, prob, tolerance, epsilon);
    sphere_detector = new pcl_project::SphereDetector(
...
DataCollector::DataCollector()
{
    ...
    typedef message_filters::sync_policies::ApproximateTime<baxter_core_msgs::EndpointState,
        sensor_msgs::PointCloud2> SyncPolicy;

    message_filters::Synchronizer<SyncPolicy> sync(
        SyncPolicy(queue_size), baxter_arm_sub, point_cloud_sub);
    ...
    baxter_arm_motion_status_sub = nh.subscribe(
        baxter_arm_motion_status_topic, 10, &DataCollector::baxterArmMotionStatusCallback, this);
    ros::spin();
}
