                        ...
                        void* viewer_void)
{
  ...
  {
    ROS_INFO_STREAM("Left mouse button released at position ("
    ...
    const pcl::visualization::PointPickingEvent& event, void* viewer_void)
{
  ...
  ROS_INFO_STREAM("Point picking event occurred. Point index="
                  << index << " coordinate ( " << x << ", " << y << ", " << z
                  << ")");

  pcl::visualization::PCLVisualizer* viewer =
      static_cast<pcl::visualization::PCLVisualizer*>(viewer_void);
  std::string text = "3D (" + utility::to_string(x) + ", " +
                     utility::to_string(y) + ", " + utility::to_string(z) + ")";
