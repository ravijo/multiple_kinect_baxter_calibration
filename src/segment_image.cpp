 ...
 * Author: Ravi Joshi
 * Date: 2018/04/18
 * Source: https://docs.opencv.org/2.4/doc/tutorials/highgui/trackbar/trackbar.html
 */
...
inline void segment_image()
{
  ...
  cv::cvtColor(image, bgr_image, cv::COLOR_BGRA2BGR, 3); // remove alpha channel
  cv::cvtColor(bgr_image, hsv_image, cv::COLOR_BGR2HSV); // convert to hsv
  bgr_image.release(); // remove bgr_image from memory

  cv::inRange(hsv_image, cv::Scalar(min_h, min_s, min_v), cv::Scalar(max_h, max_s, max_v), binary_image);
...
int main( int argc, char** argv )
{
 ...
 createTrackbar( "Maximum Hue", window_text, &max_h, slider_max, on_max_h_trackbar );
