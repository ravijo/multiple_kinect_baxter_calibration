...
int main(int argc, char** argv)
{
    ...
    else
    {
        ...
        ROS_WARN_STREAM("PCD file is not provided. Using '" << pcd_file << "' as default PCD file");
    }
    ...
    else
    {
        ...
        ROS_WARN_STREAM(
            "Sphere radius is not provided. Using " << sphere_radius << " as default value");
    }
    ...
    {
        min_h = default_min_h;
        ROS_WARN_STREAM("Minimum hue is not provided. Using " << min_h << " as default value");
    }
    ...
    else
    {
        min_s = default_min_s;
        ROS_WARN_STREAM(
            "Minimum saturation is not provided. Using " << min_s << " as default value");
    }
    ...
    {
        min_v = default_min_v;
        ROS_WARN_STREAM("Minimum value is not provided. Using " << min_v << " as default value");
    }
    ...
    {
        max_h = default_max_h;
        ROS_WARN_STREAM("Maximum hue is not provided. Using " << max_h << " as default value");
    }
    ...
    else
    {
        max_s = default_max_s;
        ROS_WARN_STREAM(
            "Maximum saturation is not provided. Using " << max_s << " as default value");
    }
    ...
    {
        max_v = default_max_v;
        ROS_WARN_STREAM("Maximum value is not provided. Using " << max_v << " as default value");
    }
    ...
    bool status = sphereDetector.segmentSphere(&pcd_viewer, cloud, segmented_cloud, sphere_coff);

    pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZRGB> red(
        segmented_cloud, 255, 0, 0);
    ...
    if (status)
    {
        ...
        pcl::PointXYZ detected_sphere(
            sphere_coff.values[0], sphere_coff.values[1], sphere_coff.values[2]);
        seg_viewer.addSphere(
            detected_sphere, sphere_coff.values[3], 0.2, 0.3, 1.0, "detected_sphere");
